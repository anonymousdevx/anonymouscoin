...
uint256 hashGenesisBlock("0x00000dcd9c563a647e3923071af44ac337bea611193d19f6b6b3a0f490dfc1cb"); //mainnet

static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // AnonymousCoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "AnonymousCoin Signed Message:\n";
...
{
    // AnonymousCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nBits, int nHeight, int64 nFees)
{
   ...
   int64 nSubsidy =5 * COIN;
	...
	else if (nHeight <= 3600 && nHeight > 1)
    {
        nSubsidy =100 * COIN;
    }
	else if (nHeight <= 7200 && nHeight > 3600)
    {
        nSubsidy =50 * COIN;
    }
	else if (nHeight <= 10800 && nHeight > 7200)
    {
        nSubsidy =25 * COIN;
    }
	else if (nHeight <= 14400 && nHeight > 10800)
    {
        nSubsidy = 10 * COIN;
    }
...
static const int64 nTargetTimespan = 100 * 60; // AnonymousCoin: 1 day
static const int64 nTargetSpacing = 2 * 60; // AnonymousCoin: 2 minutes
static const int64 nInterval = nTargetTimespan / nTargetSpacing; // 50
...
unsigned int static GetNextWorkRequired_V1(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // AnonymousCoin: This fixes an issue where a 51% attack can change difficulty at will.
...
unsigned int static DarkGravityWave(const CBlockIndex* pindexLast, const CBlockHeader *pblock) {
    /* current difficulty formula, AnonymousCoin - DarkGravity v2, written by Evan Duffield - evan@AnonymousCoin.io */
    const CBlockIndex *BlockLastSolved = pindexLast;
    const CBlockIndex *BlockReading = pindexLast;
    const CBlockHeader *BlockCreating = pblock;
    BlockCreating = BlockCreating;
...
unsigned int static DarkGravityWave3(const CBlockIndex* pindexLast, const CBlockHeader *pblock) {
    /* current difficulty formula, AnonymousCoin - DarkGravity v3, written by Evan Duffield - evan@AnonymousCoin.io */
    const CBlockIndex *BlockLastSolved = pindexLast;
    const CBlockIndex *BlockReading = pindexLast;
    const CBlockHeader *BlockCreating = pblock;
    BlockCreating = BlockCreating;
...
unsigned int static GetNextWorkRequired_V2(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
        ...
        static const unsigned int TimeDaySeconds = 60 * 100;
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckVotes) const
{
    ...
    // AnonymousCoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // AnonymousCoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block        
        const char* pszTimestamp = " 29/May/2014 The Coins coming from USA, Is Now Accepting Bitcoins";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1401334831;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 29392662;
        ...
        assert(block.hashMerkleRoot == uint256("0xd19b01fd5d2e754b29abceb5dc4f4a9d4169182ebd9f1eb05afdd9a7e1574542"));
        block.print();
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("AnonymousCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("AnonymousCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static AnonymousCoinMiner(CWallet *pwallet)
{
    printf("AnonymousCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("AnonymousCoin-miner");
    ...
    try { loop {
        ...
        printf("Running AnonymousCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("AnonymousCoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&AnonymousCoinMiner, pwallet));
}
...
/* *** BEGIN DARKSEND MAGIC - AnonymousCoin **********
    Copyright 2014, AnonymousCoin Developers 
        eduffield - evan@AnonymousCoin.io
        InternetApe - kyle@AnonymousCoin.io
*/
